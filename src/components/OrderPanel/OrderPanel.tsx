...
import { useAvailableEquity } from "@/lib/frontend/hooks/accountHooks";
import { useAsset } from "@/lib/frontend/hooks/assetHooks";
import { useCurrentMarkPrice } from "@/lib/frontend/hooks/markPriceHooks";
...
function OrderPanel() {
	...
	function close(e: MouseEvent<HTMLElement>) {
		if (dialogRef.current) {
			...
			if (dialogDimensions.width && (e.clientX < dialogDimensions.left || e.clientX > dialogDimensions.right || e.clientY > dialogDimensions.bottom || e.clientY < dialogDimensions.top)) {
	...
	if (initialMarkPriceRef.current === null) {
		initialMarkPriceRef.current = markPrice.data;
	}
	...
	return (
		<div className="bg-background-2 w-full h-full flex flex-col rounded-xl py-6 px-4 text-gray-400">
			...
			<div className="my-5 flex justify-between">
				...
				<div className="bg-background-3 px-2 rounded-sm relative">
					...
					>
						...
						<div className="flex justify-between p-2 focus-within:outline-blue-600 focus-within:outline-2 rounded-md outline-gray-700 outline-2 m-auto my-4" tabIndex={0}>
						...
						<div className="flex justify-around mt-4">
							<button onClick={cancelLeverage} className="bg-background-3 text-white px-9 py-3 rounded-xl font-semibold cursor-pointer border-white border">Cancel</button>
							<button onClick={confirmLeverage} className="bg-white text-black px-9 py-2 rounded-xl font-semibold cursor-pointer border-black border">Confirm</button>
						</div>
					</dialog>
				</div>
			</div>
			...
			{choice === "Limit" ? (
				<LimitOrderPanel asset={asset.data} availableEquity={equity} price={initialMarkPriceRef.current} side={side} leverage={leverage} />
			) : (
